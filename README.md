...
- **三种防护模式**一键切换：普通路由器 / 认证+反检测 / 反检测+梯子；
- **省配额调度**：夜间断网静默、失败退避、可一键"今日暂停认证"；
- **中文 LuCI 界面**：状态灯实时显示各组件运行情况。
...
- 切换在 **Save & Apply 和每次开机时自动生效**（幂等：切回 `normal` 会自动停掉 UA3F/OpenClash、撤掉加固规则、恢复 IPv6）。
- `proxy` 模式只负责把 OpenClash **跑起来**；订阅、节点、分流规则仍在 OpenClash 自己的界面里管理。首次进入该模式会把 `openclash-ua3f.yaml` 示例模板复制到 `/etc/openclash/config/`，不会覆盖你已有的配置。
- 未安装 UA3F 或 OpenClash 不会报错，只会在日志里降级提示（`grep mode: /var/log/campus-auth.log`）。
- 状态页顶部实时显示三个组件的开/关状态。
...
**学校有夜间断网（如 0 点断 6 点恢复）？** 把 Settings 里的静默窗口设为 `00:00`–`06:00`（默认就是）。断网期间插件完全静默，恢复后做当天第一笔登录——每笔登录都可能消耗账号的设备绑定配额，登录越少越省。

**登录被拒、日志里 `reasoncode:7`（绑定次数用完）？** 这是门户在说这个账号绑定的设备满了。插件会自动拉长重试间隔（默认 15 分钟一次，`login_holdoff` 可调），不刷屏不撞墙。恢复见下一条和 FAQ 的"绑定次数用完了怎么恢复"。

**日志里 `reasoncode:43`（"是否重新绑定至当前设备"）？** 这是门户在主动提供换绑：它附带的 bindmac 若就是路由器自己的 MAC，**v1.0.10 起插件自动确认换绑并重新登录**，无需人工（只动自己的绑定，不影响任何其他设备）；若 bindmac 是别的设备的 MAC，插件会拒绝并在日志里提示，需要你在浏览器里人工确认。
...
**验证"换 IP 是否烧配额"？** 每次登录成功都会在日志里记 `wan_ip=x.x.x.x`。连续几天对比这个 IP 和配额消耗节奏，就能确认你们学校的门户是不是按地址算绑定。
...
**踩坑了？** 先看 [docs/LESSONS.md](docs/LESSONS.md) ——25 条实机部署踩坑实录，很可能你遇到的就是其中一条。
...
会，而且是本插件帮不了的那种。很多校园门户的"设备绑定"实际按**账号×网口位置**计数：路由器在原位置掉线重连是复用绑定、不消耗配额；换一个网口再登录 = 新绑定 = 消耗配额，配额烧光就报 `reasoncode:7`（"绑定次数已用完"），且不一定按天恢复。实测案例：路由器 A 宿舍正常 → 搬 B 宿舍（同账号）被拒 → 搬回 A 原网口仍被拒。**结论：路由器固定一个网口别搬**。插件侧有两道保险：省配额策略（夜间静默、失败退避、今日暂停）把重试消耗压到最低；若门户发起 **reasoncode:43 换绑提议**（且绑定 MAC 是本机），v1.0.10 起会自动确认换绑并重新登录。

**Q：绑定次数用完了怎么恢复？**
实测两条路：① **学校 app → 增加绑定**（多数校园 app 的设备管理里都有，直接扩充名额，立刻生效）；② 若日志报的是 `reasoncode:43` 且给出的 bindmac 就是路由器自己的 MAC，插件会**自动换绑**恢复，无需人工。遇到其他 reasoncode 或两条路都走不通，再去 app 找"自助解绑设备"。另外注意：路由器断电重启后时钟可能慢几个小时，静默窗口等时间调度会跟着偏——LuCI 状态页核对时间，或 `date -s @<PC的epoch秒>` 手动校正。
