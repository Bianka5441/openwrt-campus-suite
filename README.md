...
**登录被拒、日志里 `reasoncode:7`（绑定次数用完）？** 这是门户在说这个账号绑定的设备满了。插件会自动拉长重试间隔（默认 15 分钟一次，`login_holdoff` 可调），不刷屏不撞墙。多数学校配额按天重置，第二天自动恢复。
...
会，而且是本插件帮不了的那种。很多校园门户的"设备绑定"实际按**账号×网口位置**计数：路由器在原位置掉线重连是复用绑定、不消耗配额；换一个网口再登录 = 新绑定 = 消耗配额，配额烧光就报 `reasoncode:7`（"绑定次数已用完，请在 app 上自助解绑设备"），且不一定按天恢复。实测案例：路由器 A 宿舍正常 → 搬 B 宿舍（同账号）被拒 → 搬回 A 原网口仍被拒。**结论：路由器固定一个网口别搬**；真要搬，做好去 app 解绑旧设备的准备。插件的省配额策略（夜间静默、失败退避、今日暂停）能把重试消耗压到最低，但挡不住"搬家"本身。
